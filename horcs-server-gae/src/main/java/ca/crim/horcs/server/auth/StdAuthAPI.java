package ca.crim.horcs.server.auth;
...
public class StdAuthAPI {
     ...
     */
    public Response initAuth(@Context
    HttpServletRequest request) {
        String rawAuthHeader = request.getHeader("Authorization");
        if (StringUtils.isNotBlank(rawAuthHeader)) {
            String authHeader = Base64.base64Decode(rawAuthHeader.substring("Basic ".length()));
     ...
     * @return
     */
    public Response checkAuth(@Context
    HttpServletRequest request) {
        String rawAuthHeader = request.getHeader("Authorization");
        if (StringUtils.isNotBlank(rawAuthHeader)) {
            String authHeader = Base64.base64Decode(rawAuthHeader.substring("Basic ".length()));
            ...
            if (userAndPw != null && userAndPw.length >= 1) {
                ...
                if (StringUtils.isNotBlank(hashedPw) && hashedPw.equals(hash)) {
                    ...
                    logger.info("Granting access to user {0} with token {1}", user, token);
