...
import ca.jp.secproj.utils.crypto.rsa.RsaBlockCipher;
...
public class TestRsaBlockCipher {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException,
		...
		keyManager.addKey(PRIV_KEY_NAME,
				dec.decode(PRIV_KEY.getBytes("UTF-8")), KeyType.PRIVATE, av);
		keyManager.addKey(PUB_KEY_NAME, dec.decode(PUB_KEY.getBytes("UTF-8")),
				KeyType.PUBLIC, av);
		...
		byte[] out = rbc.encrypt(IOUtils.toByteArray(inOrig),
				keyManager.getKey(PUB_KEY_NAME, av));
		outCrypt.write(out);
		inOrig.close();
		outCrypt.close();
		...
		out = rbc.decrypt(IOUtils.toByteArray(inCrypt),
				keyManager.getKey(PRIV_KEY_NAME, av));
		...
		keyManager.addKey(PRIV_KEY_NAME,
				dec.decode(PRIV_KEY.getBytes("UTF-8")),
				KeyType.PRIVATE, av);
		keyManager.addKey(PUB_KEY_NAME,
				dec.decode(PUB_KEY.getBytes("UTF-8")), KeyType.PUBLIC,
				av);
