...
public class BigIntegerMath {
	...
	/**
	 * A nonrecursive version of euclid. It returns an array answer of 3
	 * BigIntegers answer[0] is the gcd, answer[1] is the coefficient of a,
	...
	public static BigInteger[] euclid(BigInteger a, BigInteger b) throws IllegalArgumentException {
		...
		if (a.compareTo(BigInteger.ZERO) <= 0 || b.compareTo(BigInteger.ZERO) <= 0)
			throw new IllegalArgumentException("Euclid requires both arguments to be positive!");
	...
	/**
	 * Return the multiplicative inverse of a mod n. Precondition: the inverse
	 * exists.
	 * 
	 * @param a
	...
	public static BigInteger inverse(BigInteger a, BigInteger n) {
		...
		if (!rtn[0].equals(BigInteger.ONE)) {
			System.err.println("Arguments (" + a + "," + n + ") to inverse aren't relatively prime.");
		}
	...
	/**
	 * Return a triple (d, x, y) in which d, x, and y are BigIntegers, d =
	 * gcd(a,b) and a*x + b*y = d. Use the Extended Euclid to determine these
	 * values.
	 * 
	 * @param a
	 * @param b
	 *            the BigInteger arguments whose gcd is to be determined
	 * @return the triple (d,x,y) of BigIntegers in which d = gcd(a,b) and a*x +
	 *         b*y = d.
	 */
	 ...
	 * diophantine equations of the form ax+by=c. Returns an array z of 3
	 * elements; z[0] is the gcd of a and b, z[1] is x, z[2] is y.
	 * 
	 * @param a
	 * @param b
	 * @param c
	 ...
	 */
	public static BigInteger[] solveLinearDiophantine(BigInteger a, BigInteger b, BigInteger c)
			throws IllegalArgumentException {
		if (a.compareTo(BigInteger.ZERO) <= 0 || b.compareTo(BigInteger.ZERO) <= 0 || c.compareTo(BigInteger.ZERO) < 0)
		...
		if (c.mod(euclidAnswers[0]).compareTo(BigInteger.ZERO) != 0)
	...
	/**
	 * Returns a solution of x for linear congruences of the form ax congruent
	 * to b (mod m) Returns an array z; z[0] is the gcd of a and m, z[1] is the
	 * solution for x
	 * 
	 * @param a
	 * @param b
	...
	/**
	 * Implements the Rabin-Miller test. Number of different bases to try is
	 * passed in as an int If the BigInteger passes all tests, returns the
	 * probabilty it is prime as a double. Returns zero if the BigInteger is
	 * determined to be composite.
	 * 
	 * @param n
	 * @param numPasses
	 * @param sr
	...
	/**
	 * Finds simultaneous solutions to a linear system of congruences involving
	 * only one variable and multiple moduli.
	 * 
	 * @param residue
	 * @param modulus
	 * @return
	...
	/**
	 * Solves quadratic congruences ax^2+bx+c congruent to 0 mod n=pq Returns
	 * four solutions when they exist
	 * 
	 * @param a
	 * @param b
	 * @param c
	 * @param p
	 * @param q
	 * @param primeTolerance
	 * @return
	 */
	public static BigInteger[] solveQuadratic(BigInteger a, BigInteger b, BigInteger c, BigInteger p, BigInteger q,
			int primeTolerance) {
		...
		BigInteger t1 = lnr(
				term1.modPow(p.add(BigInteger.ONE).divide(FOUR), n).subtract(term2).multiply(q).multiply(qInv), n);
		BigInteger t2 = lnr(
				term1.modPow(q.add(BigInteger.ONE).divide(FOUR), n).subtract(term2).multiply(p).multiply(pInv), n);
		BigInteger t3 = lnr(term1.modPow(p.add(BigInteger.ONE).divide(FOUR), n).negate().subtract(term2).multiply(q)
				.multiply(qInv), n);
		BigInteger t4 = lnr(term1.modPow(q.add(BigInteger.ONE).divide(FOUR), n).negate().subtract(term2).multiply(p)
				.multiply(pInv), n);
		// Form the solutions
		...
		for (int i = 0; i < 4; i++) {
			x = result[i];
			if (!lnr(a.multiply(x.multiply(x)).add(b.multiply(x)).add(c), n).equals(BigInteger.ZERO))
	...
	/**
	 * Monte Carlo factorization method returns a Monte Carlo factor. An array
	 * holds a sequence of random numbers; must specify max size of this array.
	...
	public static BigInteger monteCarloFactor(BigInteger n, int maxArraySize) throws IllegalArgumentException {
		...
		for (int i = 1; i <= maxArraySize; i++) {
			m[i] = m[i - 1].multiply(m[i - 1]).add(BigInteger.ONE).mod(n);
			if (i % 2 == 0) {
	...
	public static BigInteger sqrt(BigInteger m) {
		// Uses the Newton method to find largest integer whose square does not
		// exceed m
		...
		if (diff < 0)
			throw new IllegalArgumentException("Cannot compute square root of a negative integer!");
		...
		// Begin with an initial guess-the square root will be half the size of
		// m
		// Make a byte array at least that long, & set bit in the high order
		// byte
		...
		// As long as our new approximation squared exceeds m, we continue to
		// approximate
	...
	public static BigInteger logExhaustiveSearch(BigInteger base, BigInteger residue, BigInteger modulus) {
		// This algorithm solves base^x = residue (mod modulus) for x using
		// exhaustive search
		BigInteger basePow = BigInteger.valueOf(1);
		BigInteger j;
		for (j = BigInteger.valueOf(1); j.compareTo(modulus) < 0; j = j.add(BigInteger.ONE)) {
			basePow = basePow.multiply(base).mod(modulus);
			if (basePow.equals(residue))
				break;
		}
		if (j.equals(modulus))
	...
	public static BigInteger logBabyStepGiantStep(BigInteger base, BigInteger residue, BigInteger modulus) {
		// This algorithm solves base^x = residue (mod modulus) for x using baby
		// step giant step
