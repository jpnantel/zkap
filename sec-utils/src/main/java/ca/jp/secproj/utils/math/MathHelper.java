...
public class MathHelper {
	 ...
	 * @param result
	 */
	private static void decomposeRec(int nombre, int[] facteurs, int[] current,
			int posCurrent, int startPos, List<int[]> result) {
		if (nombre == 0) {
			...
			for (int i = startPos; i < facteurs.length; i++) {
				if (nombre >= facteurs[i]) {
					...
					decomposeRec(nombre - facteurs[i], facteurs, current,
							posCurrent + 1, i, result);
				}
			}
		}
	}
	...
	public static int[] createLogarithmicRefTable(int size) {
		...
		for (int i = 0; i < logTable.length; i++) {
			logTable[i] = (int) (size - Math.ceil(size
					* (Math.log(logTable.length - i) / maxLog)));
		}
		int pos = -1;
