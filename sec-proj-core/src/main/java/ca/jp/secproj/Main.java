...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			ctx = new ClassPathXmlApplicationContext(
					"/ca/crim/horcs/applicationContext.xml");
	...
	private static void testJpake(ClassPathXmlApplicationContext ctx) {
		...
		logger.info("-------------------");
		JPakeAuthTest authTest = ctx.getBean("horcsAuthJPakeTest",
				JPakeAuthTest.class);
	...
	private static void testJPakeNoRound3(ClassPathXmlApplicationContext ctx) {
		...
		logger.info("-------------------");
		JPakeAuthTestNoRound3 authTest = ctx.getBean("horcsAuthJPakeTest2",
				JPakeAuthTestNoRound3.class);
		loopTest(20, authTest);
	}
	...
	private static void testStd(ClassPathXmlApplicationContext ctx) {
		...
		logger.info("-------------------");
		StdAuthTest authTest = ctx.getBean("horcsStdAuthTest",
				StdAuthTest.class);
		loopTest(20, authTest);
	}
	...
	private static void testStdSaltMem(ClassPathXmlApplicationContext ctx) {
		...
		logger.info("-------------------");
		StdAuthTestSaltMem authTest = ctx.getBean("horcsStdAuthTestSaltMem",
				StdAuthTestSaltMem.class);
		loopTest(20, authTest);
	}
	...
	private static void loopTest(int nbLoops, AuthTest authTest) {
		...
		logger.info("Mean time (s): "
				+ MathHelper.round((float) avg / 1000f, 4));
		logger.info("Standard deviation: "
				+ MathHelper.round(stdDev / 1000f, 4));
	}
